...
// ----------------------------------------------------------------------------------
// POST /uploading  -> file upload (protected)
if ($method === 'POST' && ($request[0] ?? '') === 'upload') {
  $user_id = getAuthenticatedUser();  // from jwt_handler.php
  if (!$user_id) {
    http_response_code(401);
...
if (
    ...
    // Finding Patient and map to user_id
    $user_id = null;
    ...
    // Creating user if they do not exist
    if (!$user_id) {
